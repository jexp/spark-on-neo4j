...
class Neo4j(val sc : SparkContext) extends QueriesDsl with PartitionsDsl with ResultDsl {
  ...
  def inTx[T](cb: (GraphDatabaseService) => T): T = {
    val db = Neo4j.db
    val tx = db.beginTx()
    try {
      val result = cb.apply(db)
      tx.success()
      result
    } finally {
      tx.close()
...
object Executor {
  ...
  def execute(sc: SparkContext, query: String, parameters: Map[String, AnyRef]): CypherResult = {
    execute(Neo4j.db, query, parameters)
  }
  private def rows(result : Result) = {
    var i  = 0
    while (result.hasNext) i=i+1
    i
  }

  def execute(db:GraphDatabaseService, query: String, parameters: Map[String, Any]): CypherResult = {
    val result: Result = db.execute(query, toJava(parameters))
    if (!result.hasNext) {
      result.close()
      return new CypherResult(new StructType(), Iterator.empty)
    }
    val peek = result.next()
    ...
    val keys = peek.keySet().asScala
    val fields = keys.map( k => (k, peek.get(k).getClass())).map( keyType => CypherTypes.field(keyType)).toList
    val schema = StructType(fields)

    val it = (Iterator(peek) ++ result.asScala).map((record) => {
      val row = new Array[Any](keyCount)
        var i = 0
        while (i < keyCount) {
          row.update(i, record.get(i))
          i = i + 1
      }
      if (!result.hasNext) {
        result.close()
...
class Neo4jRDD(@transient sc: SparkContext, val query: String, val parameters: Map[String,Any] = Map.empty, partitions : Partitions = Partitions() )
  ...
  override def compute(partition: Partition, context: TaskContext): Iterator[Row] = {
    ...
    Executor.execute(Neo4j.db, query, parameters ++ neo4jPartition.window).sparkRows
  }
