...
private[deploy] class ExecutorRunner(
  ...
  private def fetchAndRunExecutor() {
    try {
      // Launch the process
      val cmd: Command = appDesc.command

      val clazz = cmd.mainClass
      val args = cmd.arguments.map(substituteVariables)
      ...
      */
      val driverUrl = args(1)
      val cores: Int = Runtime.getRuntime.availableProcessors()
      val userClassPath: Seq[Nothing] = Seq.empty
      val formattedCommand = args.mkString(" ")
...
//      val exitCode = process.waitFor()
      CoarseGrainedExecutorBackend.run(driverUrl,execId.toString,host,cores,appId,Option(workerUrl), userClassPath)
