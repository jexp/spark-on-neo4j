...
class Neo4jSparkTest {
  ...
  /*
  val store: File = new File("target/test-db")
  store.mkdirs()
  val sparkHome: File = new File(store,"spark")
  sparkHome.mkdirs()
  new File(sparkHome,"RELEASE").createNewFile()
  new File(sparkHome,"jars").mkdirs()
  */

  @Before
  @throws[Exception]
  def setUp() {
    val masterUrl = "spark://Ich.local:7077"
    graphDb = new TestGraphDatabaseFactory().newImpermanentDatabaseBuilder().setConfig("spark.master",masterUrl).newGraphDatabase()
    graphDb.execute(FIXTURE).close()
    conf = new SparkConf().setAppName("neoTest").setMaster(masterUrl) // .set("spark.driver.allowMultipleContexts", "true")
    sc = SparkContext.getOrCreate(conf)
  }

  @Test def runCoreAPI() {
    val count = graphDb.execute("MATCH (n:Person) RETURN count(*) as c").columnAs[Long]("c").next()
    val people = Neo4j(sc).inTx(db => db.findNodes(Label.label("Person")).asScala.size)
    assertEquals(count,people)
  }
